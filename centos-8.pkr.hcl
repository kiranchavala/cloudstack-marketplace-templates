...
variable "iso_checksum" {
  type    = string
  default = "sha256:9602c69c52d93f51295c0199af395ca0edbe35e36506e32b8e749ce6c8f5b60a"
}
...
variable "url" {
  type    = string
  default = "http://compute.o.auroraobjects.eu/iso/CentOS-8.5.2111-x86_64-boot.iso"
}
...
source "qemu" "centos-qemu" {
  accelerator      = "kvm"
  boot_command     = ["<tab> text ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/kickstart/centos-8.ks<enter><wait>"]
  communicator     = "ssh"
  cpus             = 2
...
build {
  ...
  provisioner "shell" {
    ...
    scripts         = ["${path.root}/scripts/centos-8/post.sh"]
  }
...
post-processors {
    ...
    post-processor "shell-local" {
     ...
     post-processor "shell-local" {
      ...
      script           = "${path.root}/scripts/centos-8/vmdk-ova.sh"
