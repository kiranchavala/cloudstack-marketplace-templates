...
source "qemu" "ubuntu-qemu" {
  ...
  format                 = "qcow2"
  headless               = true
  http_directory         = "${path.root}/files/ubuntu-18.04"
  http_port_max          = 8100
  http_port_min          = 8000
...
build {
  ...
  provisioner "file" {
    ...
    source      = "${path.root}/files/apt/ubuntu-18.04.sources"
  }
  ...
  provisioner "shell" {
    ...
    scripts         = ["${path.root}/scripts/ubuntu-18.04/remove-swap.sh"]
  }

  provisioner "shell" {
    execute_command = "sudo sh '{{ .Path }}'"
    scripts         = ["${path.root}/scripts/ubuntu-18.04/post.sh"]
  }
  ...
  post-processors {
    ...
    post-processor "shell-local" {
     ...
     post-processor "shell-local" {
      ...
      script           = "${path.root}/scripts/ubuntu-18.04/vmdk-ova.sh"
