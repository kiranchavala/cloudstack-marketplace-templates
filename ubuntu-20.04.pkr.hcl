...
build {
  ...
  provisioner "file" {
    ...
    source      = "${path.root}/files/apt/ubuntu-20.04.sources"
  }
  ...
  provisioner "file" {
    ...
    source      = "${path.root}/files/ubuntu-20.04/01-netcfg.yaml"
  }
  ...
  provisioner "shell" {
    ...
    scripts         = ["${path.root}/scripts/ubuntu-20.04/remove-swap.sh"]
  }

  provisioner "shell" {
    execute_command = "sudo sh '{{ .Path }}'"
    scripts         = ["${path.root}/scripts/ubuntu-20.04/post.sh"]
  }
  ...
  post-processors {
    ...
    post-processor "shell-local" {
     ...
     post-processor "shell-local" {
      ...
      script           = "${path.root}/scripts/ubuntu-20.04/vmdk-ova.sh"
